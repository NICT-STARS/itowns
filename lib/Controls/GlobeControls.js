...
var GlobeControls = /*#__PURE__*/function (_THREE$EventDispatche) {
  ...
  function GlobeControls(view, placement) {
    var _options$minPolarAngl, _options$minPolarAngl2;
    ...
    _this.minZoom = options.minZoom || 0;
    _this.maxZoom = options.maxZoom || Infinity; // Set to true to disable this control

    _this.rotateSpeed = options.rotateSpeed || 0.25; // Set to true to disable this control

    _this.keyPanSpeed = options.keyPanSpeed || 7.0; // pixels moved per arrow key push
    // How far you can orbit vertically, upper and lower limits.
    // Range is 0 to Math.PI radians.
    // TODO Warning minPolarAngle = 0.01 -> it isn't possible to be perpendicular on Globe

    _this.minPolarAngle = THREE.MathUtils.degToRad((_options$minPolarAngl = options.minPolarAngle) !== null && _options$minPolarAngl !== void 0 ? _options$minPolarAngl : 0.5);
    _this.maxPolarAngle = THREE.MathUtils.degToRad((_options$minPolarAngl2 = options.minPolarAngle) !== null && _options$minPolarAngl2 !== void 0 ? _options$minPolarAngl2 : 86); // How far you can orbit horizontally, upper and lower limits.
    // If set, must be a sub-interval of the interval [ - Math.PI, Math.PI ].

    _this.minAzimuthAngle = options.minAzimuthAngle ? THREE.MathUtils.degToRad(options.minAzimuthAngle) : -Infinity; // radians

    _this.maxAzimuthAngle = options.maxAzimuthAngle ? THREE.MathUtils.degToRad(options.maxAzimuthAngle) : Infinity; // radians
    ...
    _this.enableDamping = options.enableDamping !== false;
        ...
        case this.states.MOVE_GLOBE_ARROUND_POLE:
          {
            if (minDistanceZ < 0) {
              ...
              var _translate = this.minDistanceCollision * (1.0 - minDistanceZ / this.minDistanceCollision);

              cameraTarget.translateY(_translate);
              this.camera.position.setLength(this.camera.position.length() + _translate);
            }
    ...
    value: function handleEndMovement() {
      ...
      if (this.enableDamping) {
        if (event.previous === this.states.ORBIT && (sphericalDelta.theta > EPS || sphericalDelta.phi > EPS)) {
          ...
          this.player.setCallback(function () {
            _this3.update(_this3.states.MOVE_GLOBE_ARROUND_POLE);
          }); // animation since mouse up event occurs less than 50ms after the last mouse move
